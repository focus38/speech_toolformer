...
Итоговый проект по результатам обучения на 3 семестре курса DLS Speech.

## Про что проект? Краткое описание сути и самый интересный момент в проекте

Построить голосового ассистента на базе Gemma-3n (или LFM2-Audio/другой до ~4B) с поддержкой вызова инструментов:
ассистент слушает запрос на русском или английском, решает, нужен ли вызов инструмента, выдаёт форматированный вызов (JSON/XML),
бэкэнд выполняет его и возвращает обоснованный ответ текстом (или озвученный).

Самое интересное: сравнить разные пайпланы работы со звуком (Audio - model - ASR - model - tool-call или Audio - model - tool-call и другие)
и понять, сколько дообучения реально требуется, чтобы из речи стабильно получать корректные tool-calls.
...
- Креативность выбора инструмента (2 балла)
- System-prompt tuning и оценка модели на следование инструкциям (2 балла): чёткая схема инструмента определена; сделана оценка на тест-сете с примерами.
- Метрики по текстовому tool-use (пайплайн A) (3 балла): accuracy, precision/recall, false alarm rate, доля ответов моедели, которые удается распарсить; корректная интерпретация результатов.
- Дизайн синтетического датасета (4 балла): понятное описание; сбалансированные текст+аудио; пример запросов «без инструмента»;
- ASR бенчмаркинг (пайплайн B) (3 балла): WER + короткий разбор ошибок
- Бенчмаркинг аудио пайплайнов C–D (3 балла)
- Выбор лучшего пайплайна (2 балла): обоснование выбора по метрикам.
- Финальный отчёт (3 балла): краткие выводы и фейл-кейсы.