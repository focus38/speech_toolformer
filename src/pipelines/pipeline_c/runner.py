...
class PipelineCAudioAlignmentError(ValueError):
    """Raised when Pipeline C inputs do not describe the same fixed audio test split."""
...
def _load_test_examples(dataset_path: str | Path) -> list[DatasetExample]:
    ...
    if non_test_ids:
        raise PipelineCAudioAlignmentError(
            "Pipeline C must run on the fixed test split; "
    ...
    with Path(metadata_path).open(encoding="utf-8") as file:
        for line_number, line in enumerate(file, start=1):
            ...
            if example_id in samples_by_id:
                raise PipelineCAudioAlignmentError(f"Duplicate audio metadata for example id: {example_id}")
...
def _metadata_for_examples(
    ...
    for example in examples:
        ...
        if sample is None:
            raise PipelineCAudioAlignmentError(f"Missing audio metadata for example id: {example.id}")
        if sample.transcript != example.user_text:
            raise PipelineCAudioAlignmentError(f"Transcript mismatch for example id: {example.id}")
        if sample.language.value != example.language.value:
            raise PipelineCAudioAlignmentError(f"Language mismatch for example id: {example.id}")
        audio_path_parts = Path(sample.audio_path).parts
        if len(audio_path_parts) < 2 or audio_path_parts[-2] != Split.TEST.value:
            raise PipelineCAudioAlignmentError(f"Audio metadata is not in the test split: {sample.audio_path}")
    ...
    if extra_ids:
        ...
        raise PipelineCAudioAlignmentError(f"Audio metadata contains non-test or unknown ids: {preview}{suffix}")
    return pairs
...
def run_pipeline_c(
    *,
    ...
    for example, sample in tqdm(pairs, desc="Processing pipeline C", unit="audio item"):
        ...
        result = inference.generate_joint(audio)
...
__all__ = ["PipelineCAudioAlignmentError", "run_pipeline_c"]
