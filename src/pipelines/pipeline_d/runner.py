...
class PipelineDAudioAlignmentError(ValueError):
    """Raised when Pipeline D inputs do not describe the same fixed audio test split."""
...
def _load_test_examples(dataset_path: str | Path) -> list[DatasetExample]:
    ...
    if non_test_ids:
        raise PipelineDAudioAlignmentError(
            "Pipeline D must run on the fixed test split; "
    ...
    with Path(metadata_path).open(encoding="utf-8") as file:
        for line_number, line in enumerate(file, start=1):
            ...
            if example_id in samples_by_id:
                raise PipelineDAudioAlignmentError(f"Duplicate audio metadata for example id: {example_id}")
...
def _metadata_for_examples(
    ...
    for example in examples:
        ...
        if sample is None:
            raise PipelineDAudioAlignmentError(f"Missing audio metadata for example id: {example.id}")
        if sample.transcript != example.user_text:
            raise PipelineDAudioAlignmentError(f"Transcript mismatch for example id: {example.id}")
        if sample.language.value != example.language.value:
            raise PipelineDAudioAlignmentError(f"Language mismatch for example id: {example.id}")
...
def run_pipeline_d(
    *,
    ...
    for example, sample in tqdm(pairs, desc="Processing pipeline D", unit="audio item"):
        ...
        audio_result = audio_inference.transcribe(audio)
        transcript = audio_result.transcript or ""
        prompt = render_tool_call_prompt(transcript)
        text_result = text_inference.generate(prompt)
        latency_seconds = perf_counter() - started_at
        records.append(
            make_prediction(
                example_id=example.id,
                pipeline=Pipeline.D,
                model_name=f"{audio_result.model_name} -> {text_result.model_name}",
                prompt_version=text_result.prompt_version,
                raw_output=text_result.raw_output,
...
__all__ = ["PipelineDAudioAlignmentError", "run_pipeline_d"]
